...
def scrape_all_profiles_from_university(
    ...
    while profiles_is_present:
        ...
        for profile in select.css(".gs_ai_chpr"):
            ...
            cited_by = re.search(
                r"\d+", profile.xpath('//div[@class="gs_ai_cby"]').get()
...
print(
    scrape_all_profiles_from_university(
        label="Deep_Learning", university_name="Harvard University"
    )
)
# print(json.dumps(scrape_all_profiles_from_university(label="Deep_Learning", university_name="Harvard University"), indent=2))


def serpapi_scrape_all_profiles_from_university(
    label: str, university_name: str
) -> list[dict[str]]:
    params = {
    ...
    while profiles_is_present:
        ...
        for profile in profile_results["profiles"]:
            ...
            print(
                f'Currently extracting {profile["name"]} with {profile["author_id"]} ID.'
            )
            ...
            if "next" in profile_results.get("pagination", []):
                ...
                search.params_dict.update(
                    dict(
                        parse_qsl(
                            urlsplit(
                                profile_results.get("pagination").get("next")
                            ).query
                        )
                    )
                )
